function [dets, boxes, info] = gdetect(pyra, model, thresh, bbox, overlap, chunksize)
...
model = filterresponses(model, pyra, latent, bbox, overlap, chunksize);
...
for level = model.interval+1:max(pyra.valid_levels)    
...
function model = filterresponses(model, pyra, latent, bbox, overlap, chunksize)
% model    object model
...
%   chunksize = ceil(length(filters)/32);
  ...
  for j=1:chunksize:length(filters)
      start = j;
      last = min(j+chunksize-1, length(filters));
      r(start:last) = fconv(pyra.feat{level}, filters, start, last);
  end
  % end Jan's hands' job :)
...
if ~latent
  ...
  % changed by Leonid
  levels = levels(pyra.valid_levels);
else
  levels = [];
