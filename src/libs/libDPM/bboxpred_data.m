...
try
  ...
  for c = 1:length(model.rules{model.start});
      cDir = [visDir '/' padZeros(num2str(c-1),4)];
      if (~exist(cDir, 'dir'))
          mkdir(cDir);
      end
  end
...
%       continue;
    ...
    if ~isempty(det)
      ...
      compidx(i) = c;
      crop = im(bbox(1,2):bbox(1,4),bbox(1,1):bbox(1,3),:);
      [p,imgname,e] = fileparts(pos(i).im);
      fname = [visDir '/' padZeros(num2str(c-1),4) '/' imgname '.png'];
      imwrite(crop,fname);
    end
  end
  dets = cell(1,nrules);
  ...
  save([cachedir name '_compidx'], 'compidx');
end
